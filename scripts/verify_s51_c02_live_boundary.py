...
def main() -> int:
    if not os.environ.get("KEYHOLE_MCP_URL") or not os.environ.get("KEYHOLE_MCP_TOKEN"):
        print(json.dumps({
            "success": False,
            "skipped": True,
            "reason": "live proof not performed: KEYHOLE_MCP_URL and KEYHOLE_MCP_TOKEN are required",
        }, indent=2))
        return 0

    repo = REPO_ROOT / "my-first-app"
    try:
        client = GovernedFirstAppClient.from_env()
